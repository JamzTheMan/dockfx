...
/**
 * Base class for a dock pane that provides the layout of the dock nodes. Stacking the dock nodes to the center in a TabPane will be added in a future release. For now the DockPane uses the relative
...
public class DockPane extends StackPane
	...
	/**
	 * Whether or not this dock pane allows the docking of dock nodes from external sources (i.e., other dock panes).
	 */
	...
	/**
	 * The timeline used to animate the borer of the docking area indicator shape. Because JavaFX has no CSS styling for timelines/animations yet we will make this private and offer an accessor for
	...
	public class DockPosButton extends Button {
		 ...
		 *
		 * @param dockRoot
		 *            Whether this indicator button is used for docking a node relative to the root of the dock pane.
		 */
	...
	/**
	 * Indicates whether or not the dock pane is in exclusive mode (the default). In exclusive mode, the dock pane ignores dock nodes dragged from other dock panes, and dock nodes dragged from this
	 * dock pane are ignored by other dock panes.
	 * 
	...
	/**
	 * A wrapper to the type parameterized generic EventHandler that allows us to remove it from its listener when the dock node becomes detached. It is specifically used to monitor which dock node in
	 * this dock pane's layout we are currently dragging over.
	 *
	 * @since DockFX 0.1
	 */
	private class DockNodeEventHandler implements
		 ...
		 *
		 * @param node
		 *            The node that is to listen for docking events and report to the encapsulating docking pane.
		 */
	...
	/**
	 * Dock the node into this dock pane at the given docking position relative to the sibling in the layout. This is used to relatively position the dock nodes to other nodes given their preferred
	 * size.
	 *
	 * @param node
	...
	public void storePreference(String filePath) {
		...
		for (int i = 0; i < floatingNodes.size(); i++) {
			...
			ContentHolder floatingNode = new ContentHolder(floatingNodes.get(i)
					.getTitle(),
