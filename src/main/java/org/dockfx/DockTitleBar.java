...
public class DockTitleBar extends HBox
	...
	/**
	 * The mouse location of the original click which we can use to determine the offset during drag. Title bar dragging is asynchronous so it will not be negatively impacted by less frequent or
	...
	/**
	 * Traverse the scene graph for all open stages and pick an event target for a dock event based on the location. Once the event target is chosen run the event task with the target and the previous
	 * target of the last dock event if one is cached. If an event target is not found fire the explicit dock event on the stage root if one is provided.
	 * 
	 * @param location
	 ...
	 * @param explicit
	 *            The explicit event to be fired on the stage root when no event target is found.
	 */
	...
	public void handle(MouseEvent event) {
		if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
			...
			DockEvent dockEnterEvent = new DockEvent(this,
					...
					null,
					this.getDockNode());
			DockEvent dockOverEvent = new DockEvent(this,
					DockEvent.NULL_SOURCE_TARGET,
					DockEvent.DOCK_OVER,
					event.getX(),
					event.getY(),
					...
					null,
					this.getDockNode());
			DockEvent dockExitEvent = new DockEvent(this,
					DockEvent.NULL_SOURCE_TARGET,
					DockEvent.DOCK_EXIT,
					event.getX(),
					event.getY(),
